...
import { FaHome, FaList, FaRegEdit, FaBook, FaCog } from 'react-icons/fa';
...
const BottomNav = () => {
  const { user } = useAuth();
  const isAdmin = canAccessWorshipHeadDashboard(user);
      ...
      <div className="bg-gray-900 rounded-full px-6 py-4 shadow-lg">
        <div className="flex items-center space-x-8">
          ...
          {isAdmin && (
            ...
            >
              {({ isActive }) => (
                <FaCog className={`text-lg transition-colors duration-200 ${
                  isActive ? "text-gray-900" : "text-white"
                }`} />
              )}
            </NavLink>
